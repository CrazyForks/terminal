...
| SQLite replaces per-session JSON files as primary state. | `crates/browser-use-store` migrations create `sessions`, `events`, `artifacts`, `runs`, `agent_edges`, `agent_messages`, and `app_settings`; tests cover store/session/events/import/export and checked-in golden event fixtures. | Done |
...
| Terminal UI was manually tested. | `/tmp/but-goal-final-tui`, `/tmp/but-goal-browser-overlay-tui`, and `/tmp/but-current-final-pty` PTY runs inspected; current deterministic dumps under `/tmp/but-current-tui-audit` cover setup, ready, running, result, browser, history, actions, help, developer, and stopped states; SQLite evidence recorded in `docs/rewrite-verification.md`. | Done |

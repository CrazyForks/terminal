...
### ▶ S-goals — IN PROGRESS
- [ ] WP-S-goals — goals/budget: event-sourced goal state + token-budget accounting (input incl. cached + max(output,0)) + steering events. Codex goals/tasks + legacy browser-use-core goals. NEW module `goals/*` + pub mod line. Network-free tests.

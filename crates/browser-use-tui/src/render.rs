...
use browser_use_protocol::{
    HistoryRow, SessionMeta, TelemetrySummary, TranscriptTurn, WorkbenchState,
};
...
pub(crate) fn native_scrollback_lines(app: &mut App, width: u16) -> Result<Vec<Line<'static>>> {
    ...
    if surface == Surface::Main {
        lines.extend(match product_state {
            ...
            ProductState::Running => running_lines(&state, body_width),
...
fn render_composer(
    ...
    let max_lines = chunks[1].height.max(1) as usize;
    frame.render_widget(
    ...
    if chunks[1].width > 0 && chunks[1].height > 0 {
        ...
        frame.set_cursor_position(Position {
            x: chunks[1]
                .x
                .saturating_add(cursor_x.min(chunks[1].width.saturating_sub(1))),
            y: chunks[1]
                .y
                .saturating_add(cursor_y.min(chunks[1].height.saturating_sub(1))),
        });
    }
}

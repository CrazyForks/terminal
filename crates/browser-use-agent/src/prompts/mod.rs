...
pub fn browser_mode_instruction(mode: &str) -> String {
    ...
    match normalized.as_str() {
        "local" | "local-chrome" => concat!(
            ...
            "For tasks that depend on an existing login, account, cookies, or a private web app/domain where the user's existing session matters, choose the local Chrome profile before the first connect. Run `browser profile suggest --domain <domain>` when the domain is known, otherwise run `browser local profiles`. If more than one plausible profile is returned and no remembered profile clearly applies, ask the user which profile/window to use, wait for their answer, remember/use that profile, make sure that profile's Chrome window is open or targeted when Chrome is not already running in that profile, then run `browser connect local`. For public/no-login tasks, connect directly.\n\n",
            "After `browser connect local`, the runtime's `browser connect local` / `browser status --json` output is the source of truth. If it reports `status: \"needs-user-action\"` with multiple reachable `candidates`, ask the user which candidate/browser/profile to attach or choose the clearly task-relevant one, then run `browser connect local --candidate <id>`.\n\n",
            ...
            "`browser local profiles`, `browser profile suggest`, and `browser local setup` are safe to run before connecting; they inspect local profile/setup state and do not themselves attach to a page.\n\n",
            ...
            "      1. If the task is login-sensitive and no profile was chosen yet, run `browser local profiles` before launching a generic Chrome window. If multiple profiles are plausible, ask the user which profile/window to use and wait for their answer.\n",
            "      2. When the user chooses a profile, remember/use that profile, then open that chosen profile's Chrome window before retrying `browser connect local`. Use the target URL when known, or `https://<domain>` when only the domain is known, so Chrome does not create a blank tab. Do not launch generic Chrome first for this path, because that can start the wrong profile.\n",
            "      3. If the task is not login-sensitive, or the browser/profile is already obvious, use the `shell` tool to open Chrome — DO NOT ask the user to do this manually. macOS: `open -a \"Google Chrome\"`. Linux: `google-chrome &` (or whichever Chromium binary is on PATH — `chromium`, `brave-browser`, `microsoft-edge`). Windows: `cmd /c start chrome`. Only fall back to asking the user if the shell command errors AND you can't recover. Give Chrome a moment to start (~1–2s), then retry `browser connect local`.\n\n",
            ...
            "      1. If more than one local profile/window could receive the chrome://inspect page, run `browser local profiles`, ask which profile to use, and remember/use that profile. If the browser/profile is obvious from the candidate or only one profile is available, proceed silently.\n",
            ...
            "  C) Candidate fields say `browser_running: true` AND `remote_debugging_enabled: true` BUT the connect ATTEMPT's top-level response is `state: \"permission-blocked\"` (or `raw_error` mentions HTTP 403 / Forbidden). Routing this case from the attempt's state is correct — the candidate fields alone can't tell you Chrome will reject the WebSocket; you only find out by trying. The on-disk toggle is already on; Chrome is showing (or has queued) its per-session \"Allow remote debugging?\" popup and waiting on the user. DO NOT touch chrome://inspect, DO NOT tell the user to tick the checkbox — it's already ticked, that'd be confusing. Instead:\n",
            "      1. If the blocked candidate identifies the profile/window, focus that path. Otherwise, if multiple profile windows are plausible, run `browser local profiles`, ask which profile the user wants to use, and remember/use that profile. Only open a profile window if you need to help the user find the popup.\n",
            "      2. Tell the user: \"Chrome is showing an 'Allow remote debugging?' popup somewhere — check each open Chrome window and click Allow. Reply when done.\" Mention that with multiple profile windows the popup may appear in a window they aren't looking at.\n",
            ...
            "If the first connection succeeds after the user already chose a profile, continue with page work in the connected browser. If the task is login-sensitive and you connected without choosing a profile, verify the page is in the expected account/session before taking irreversible actions; if it is clearly the wrong profile, ask the user and switch by opening the chosen profile window and reconnecting.\n\n",
            "What to DO when the user chooses a local profile:\n",
            "  1. Record the choice with `browser profile remember --domain <domain> --profile <profile-id>` when the domain is known, or `browser profile use <profile-id>` otherwise. Do not open an empty Chrome window just because a profile was selected.\n",
            "  2. Then run `browser connect local` (or retry it). If you need to open a selected profile window because Chrome is not running, setup needs a specific profile, the permission popup is hard to find, or you must guarantee a new tab in that profile, use the profile's `profile_dir` field from the `browser local profiles` response. Include the target URL when known, or `https://<domain>` when only the domain is known, so Chrome does not create a blank tab. macOS: `open -na \"Google Chrome\" --args --profile-directory=\"<profile_dir>\" \"https://target.example\"` (the `-n` is important when a new profile-specific window is required). Linux: `google-chrome --profile-directory=\"<profile_dir>\" \"https://target.example\" &`. Windows: `cmd /c start chrome --profile-directory=\"<profile_dir>\" \"https://target.example\"`. Quote the values because `profile_dir` may contain spaces.\n",
            "  3. For page work, prefer tabs in the chosen profile's browser context. If the connected tab is clearly in the wrong account/session, stop and ask rather than continuing in the wrong profile.\n\n",

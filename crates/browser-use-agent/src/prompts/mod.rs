...
pub fn browser_mode_instruction(mode: &str) -> String {
    ...
    match normalized.as_str() {
        "local" | "local-chrome" => concat!(
            ...
            "`browser local profiles`, `browser profile suggest`, `browser profile use`, and `browser local setup` are safe to run before connecting; they inspect or update local profile/setup state and do not themselves attach to a page.\n\n",
            ...
            "      2. If a default profile is already set and the connect output includes `profile_recovery_command`, run that exact `browser local open --profile ...` command — DO NOT use a bare shell launch because that can open Chrome's profile picker instead of the saved profile. Give Chrome a moment to start (~1–2s), then retry `browser connect local`. Only fall back to the `shell` tool if `browser local open` errors AND you can't recover.\n",
            ...
            "      1. If the blocked output includes `profile_recovery_command` and the popup is not visible, run that exact `browser local open --profile ...` command to focus/open the saved profile window. Do not open Chrome generically; generic launches can show the profile picker.\n",

...
pub fn browser_mode_instruction(mode: &str) -> String {
    ...
    match normalized.as_str() {
        "local" | "local-chrome" => concat!(
            ...
            "      2. IMMEDIATELY after launching Chrome, call `browser local profiles` and check the result. If multiple profiles are returned, ASK the user which profile to use (show their `display_name` / `profile_name`) BEFORE you do anything else — including retrying the connect. Knowing the right profile up front matters because (a) different profiles have different cookies/logins/extensions, (b) the per-session 'Allow remote debugging?' popup in case C typically appears in only ONE window, so the user needs to know which window to look at. Wait for their answer.\n",
            "      3. Give Chrome a moment to start (~1–2s), then retry `browser connect local`. DO NOT jump to case B without retrying — most stale-port states resolve to fully connectable after Chrome restarts because permission is already granted on disk.\n\n",
            ...
            "      1. FIRST run `browser local profiles`. If multiple profiles are returned, ASK the user which to use and wait. If they pick one, follow 'What to DO with the chosen profile' below (open the chosen profile's window via shell) BEFORE moving on. If only one profile, proceed silently. This must happen before chrome://inspect so the toggle and the per-session popup land in the right profile's window.\n",
            ...
            "      1. FIRST run `browser local profiles`. If multiple profiles, ASK the user which to use and wait. If they pick one, follow 'What to DO with the chosen profile' below (open the chosen profile's window via shell) BEFORE the popup hand-holding — the per-session popup typically appears in only ONE window, so opening the chosen profile's window brings that target into focus.\n",
            "      2. Tell the user: \"Chrome is showing an 'Allow remote debugging?' popup somewhere — check each open Chrome window (multi-profile setups put it in only one) and click Allow. Reply when done.\" Mention that with multiple profile windows the popup often appears in a window they aren't looking at.\n",
            ...
            "First-try-succeeded path (none of A/B/C entered): before any `browser_script` page work, still run `browser local profiles`. If multiple, ASK and handle per 'What to DO with the chosen profile' below.\n\n",
            "What to DO with the chosen profile (this is the part you must not skip):\n",
            "  1. As soon as the user picks a profile from your list, use the `shell` tool to open a fresh window in THAT profile, using the profile's `profile_dir` field from the `browser local profiles` response. macOS: `open -na \"Google Chrome\" --args --profile-directory=\"<profile_dir>\"` (the `-n` is important — without it, `open` just refocuses an existing window in any profile). Linux: `google-chrome --profile-directory=\"<profile_dir>\" &`. Windows: `cmd /c start chrome --profile-directory=\"<profile_dir>\"`. Quote the value because `profile_dir` may contain spaces (e.g. `Profile 1`, `System Profile`).\n",
            "  2. That window is now the one Chrome will pop the per-session 'Allow remote debugging?' prompt in if it shows up — and the one whose tab CDP will land on after `browser connect local`. Without this step the user has no way to know which window the popup will appear in, and `browser_script` may end up driving tabs in the wrong profile.\n",
            "  3. THEN run `browser connect local` (or retry it). For page work, prefer tabs in the chosen profile's window — opening a fresh tab in that window via `browser_script` is the simplest way to guarantee profile context.\n\n",

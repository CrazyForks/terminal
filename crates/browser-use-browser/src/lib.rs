...
def js(expression, returnByValue=True):
    ...
    if "record.cells" in expression:
        ...
        assert "idRefTexts" in expression
        assert "rowHeaderTexts" in expression
        assert "headerLabelsForCell" in expression
        assert "getAttribute('headers')" in expression
        assert "aria-labelledby" in expression
        assert "colspan" in expression
        assert "rowgroup" in expression
...
assert records["records"][0]["cells"][1]["header"] == "Provider / Price"
assert records["records"][0]["cells"][1]["headers"] == ["Provider", "Price"]
...
"##,
    ...
    #[test]
    fn browser_script_extract_paginated_grid_rows_collects_pages_and_fanout() {
        let temp = tempfile::tempdir().unwrap();
        let output = run_browser_script(
            "script-paginated-grid-rows",
            temp.path(),
            temp.path().join("artifacts"),
            r##"
page_index = 0
...
def js(expression, *args, **kwargs):
    ...
    assert "earnings_release" in expression
    assert "financial_supplement" in expression
    assert "investor_presentation" in expression
    assert "period_tokens" in expression
    return {
...
"##,
        ...
        assert!(output
            .text
            .contains("investor_documents_snapshot ok"));
    }
...
snapshot = result_count_snapshot()
assert snapshot["count"] == 3
assert snapshot["best"]["total"] == 58
assert snapshot["best"]["evidence"] == "Matches 1 - 25 of 58"
assert snapshot["candidates"][1]["total_pages"] == 3
assert snapshot["candidates"][2]["total"] == 710
assert len(calls) == 1
print("result count helpers ok")
"##,
            10,
        )
        .unwrap();
        ...
        assert!(output.ok, "{:?}\n{}", output.error, output.text);
        assert!(output.text.contains("result count helpers ok"));
    }
...
"##,
    ...
    fn browser_script_location_records_snapshot_normalizes_directory_records() {
        let temp = tempfile::tempdir().unwrap();
        let output = run_browser_script(
            "script-location-records-helpers",
            temp.path(),
            temp.path().join("artifacts"),
            r##"
calls = []
...
assert len(calls) == 1
print("location records helpers ok")
"##,
            10,
        )
        .unwrap();
        ...
        assert!(output.ok, "{:?}\n{}", output.error, output.text);
        assert!(output.text.contains("location records helpers ok"));
    }
...
assert snapshot["count"] == 1
assert snapshot["controls"][0]["options"][2]["value"] == "CA"
result = select_option("state", "California", timeout=2.5)
assert result["selected"] is True
assert result["selector"] == "select[name=\"state\"]"
assert result["option_text"] == "California"
assert result["option_value"] == "CA"
assert focused == [{"selector": "select[name=\"state\"]", "timeout": 2.5}]
assert keys == ["Home", "ArrowDown", "ArrowDown", "Enter"]
assert len(calls) == 2
print("select helpers ok")
"#,
            10,
        )
        .unwrap();
        ...
        assert!(output.ok, "{:?}\n{}", output.error, output.text);
        assert!(output.text.contains("select helpers ok"));
    }
...
def http_get(url, headers=None, timeout=20.0, binary=None):
    ...
    assert "?target wdt:P69 ?institution" in query_text, query_text
    assert "?other wdt:P69 ?institution" in query_text, query_text
    assert "?other wdt:P166 ?award" in query_text, query_text
    assert "nobel" in query_text.lower(), query_text
    return json.dumps({
        "head": {"vars": ["target", "targetLabel", "institution", "institutionLabel", "other", "otherLabel", "award", "awardLabel"]},
        "results": {
            "bindings": [
                {
...
"##,
        ...
        assert!(output
            .text
            .contains("wikidata_education_award_cascade ok"));
    }

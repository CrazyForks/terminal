...
string_keyword_snapshot = sitemap_urls_snapshot("www.hostgenius.ca", keywords="rentals", limit=10)
assert string_keyword_snapshot["keywords"] == ["rentals"], string_keyword_snapshot
...
assert any(candidate["path"] == "$.data.facets" for candidate in source["candidate_arrays"]), records
...
def js(expression, *args, **kwargs):
    ...
    assert "label[for=" in expression
    assert "CSS.escape" in expression
    if "__FILL_FORM_FIELD__" in expression:
        assert "matched_text" in expression
        assert 'const needle = "postal code"' in expression
        return {
    ...
    assert "__FORM_FIELDS_SNAPSHOT__" in expression
    return [
        {
            "index": 0,
            "selector": "input[name=\"zipcode\"]",
...
assert snapshot["count"] == 1, snapshot
assert snapshot["fields"][0]["label"] == "Postal code", snapshot
result = fill_form_field("postal code", "00510", timeout=2.5)
assert result["filled"] is True, result
assert result["selector"] == "input[name=\"zipcode\"]", result
assert result["matched_text"] == "postal code zipcode", result
assert filled == [{"selector": "input[name=\"zipcode\"]", "text": "00510", "clear": True, "timeout": 2.5}], filled
assert len(calls) == 2, calls
print("form_field_helpers ok")
"#,
            10,
        )
        .unwrap();
        ...
        assert!(output.ok, "{:?}\n{}", output.error, output.text);
        assert!(output.text.contains("form_field_helpers ok"));
    }
...
assert snapshot["count"] == 1, snapshot
assert snapshot["suggestions"][0]["text"].startswith("225 Garcia"), snapshot
result = select_autocomplete("Address", "225 Garcia Ave", match_text="225 Garcia", timeout=1.0)
assert result["selected"] is True, result
assert result["field_selector"] == "input[aria-label=\"Address\"]", result
assert result["selector"] == "#addr-option", result
assert result["matched_text"] == "225 Garcia Ave, San Leandro, CA 94577", result
assert filled == [{"selector": "input[aria-label=\"Address\"]", "text": "225 Garcia Ave", "clear": True, "timeout": 1.0}], filled
assert clicked_points == [{"x": 330, "y": 198, "button": "left", "clicks": 1}], clicked_points
assert len(calls) == 3, calls
print("autocomplete_helpers ok")
"##,
            10,
        )
        .unwrap();
        ...
        assert!(output.ok, "{:?}\n{}", output.error, output.text);
        assert!(output.text.contains("autocomplete_helpers ok"));
    }
...
def js(expression, *args, **kwargs):
    ...
    assert "getBoundingClientRect" in expression
    assert "CSS.escape" in expression
    if "__DISMISS_OVERLAY__" in expression:
    ...
    assert "__OVERLAY_ACTIONS_SNAPSHOT__" in expression
    return [
        {
            "selector": "button[aria-label=\"Accept all\"]",
            "tag": "button",
...
assert snapshot["count"] == 1, snapshot
assert snapshot["actions"][0]["text"] == "Accept all cookies", snapshot
result = dismiss_overlay(prefer="accept", timeout=0)
assert result["clicked"] is True, result
assert result["matched_text"] == "Accept all cookies", result
assert clicked_points == [{"x": 312, "y": 688, "button": "left", "clicks": 1}], clicked_points
assert len(calls) == 2, calls
print("overlay_helpers ok")
"##,
            10,
        )
        .unwrap();
        ...
        assert!(output.ok, "{:?}\n{}", output.error, output.text);
        assert!(output.text.contains("overlay_helpers ok"));
    }
...
assert result["count"] == 1, result
assert result["variables"] == ["person", "personLabel", "phd", "phdLabel"], result
row = result["rows"][0]
...
assert row["personLabel"]["lang"] == "en", row

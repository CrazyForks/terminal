...
(() => {{
  ...
  const priceRe=/(?:[$€£¥]\\s?\\d[\\d.,]*(?:\\s?(?:\\/|per)\\s?(?:mo|month|mth|yr|year|day|night))?|\\d[\\d.,]*\\s?(?:€|eur|usd|gbp|dkk|sek|nok|kr|ron|aud|cad|nzd|jpy)(?:\\s?(?:\\/|per)\\s?(?:mo|month|mth|yr|year|day|night))?)/ig;
  ...
  for(const e of nodes){{const text=textOf(e);if(text.length<8)continue;const prices=uniq(text.match(priceRe)||[]),speeds=uniq(text.match(speedRe)||[]),contracts=uniq(text.match(contractRe)||[]),offers=uniq(text.match(offerRe)||[]);const hs=headings(e),ls=links(e),imgs=images(e);let score=prices.length*18+speeds.length*8+contracts.length*6+offers.length*5+hs.length*4+ls.length*2+imgs.length*2;if(/price|plan|package|offer|product|card|tile|subscription|tariff|ticket/i.test(`${{e.className||''}} ${{e.id||''}} ${{e.getAttribute('data-testid')||''}} ${{e.getAttribute('data-test')||''}}`))score+=10;if(!prices.length&&score<18)continue;const r=e.getBoundingClientRect(),key=`${{prices.join('|')}}|${{hs.join('|')}}|${{text.slice(0,120)}}`;if(seen.has(key))continue;seen.add(key);out.push({{selector:sel(e),tag:e.tagName.toLowerCase(),score,text:clean(text,1800),headings:hs,prices,speeds,contracts,offer_types:offers,links:ls,images:imgs,rect:{{x:Math.round(r.x),y:Math.round(r.y),width:Math.round(r.width),height:Math.round(r.height),in_viewport:r.bottom>=0&&r.top<=innerHeight&&r.right>=0&&r.left<=innerWidth}}}})}}
...
def openreview_notes(endpoint="search", params=None, limit=100, offset=0, timeout=20.0):
    ...
    Useful for OpenReview conference tasks: search accepted submissions,
    decisions, reviews, recommendations, authors, and forum-linked notes without
    walking the browser UI. Use endpoint="search" for /notes/search or
    endpoint="notes" for /notes with forum/invitation/signature filters.
    """

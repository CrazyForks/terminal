...
- Use `emit_output(value, label="...")` for compact structured observations that the next model turn may need, such as `page_info()`, selected DOM state, counts, samples, or API response summaries. For exploration, emit focused summaries of visible headings, links, buttons, forms, and task-relevant text; save large raw page text, HTML, accessibility trees, API dumps, or complete datasets under `outputs_dir()` instead of emitting them into the transcript unless smaller field-level extraction failed and the raw value is required immediately. Do not collect or persist cookies, localStorage, app caches, or other sensitive browser state unless the user explicitly asked for that state.

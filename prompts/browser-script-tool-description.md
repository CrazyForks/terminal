...
- After navigation, search/filter actions, SPA route changes, or downloads, call `network_resources_snapshot(keywords=[...])` to discover XHR/API, JSON, document/download, pagination, form, and export URLs before manual page walking. For JSON/API candidates, call `json_api_records(urls=[...], records_path=...)` to fetch, recursively find record arrays, and return normalized records/fields/source paths. Use promising non-JSON URLs with `http_get_many(...)`, `browser_fetch_many(...)`, or `read_document_text(...)`.

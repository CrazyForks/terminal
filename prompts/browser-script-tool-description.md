...
- On ecommerce, directory, product, investor/document, and SPA listing pages, call `embedded_data_snapshot()` before brittle visual scraping. It extracts JSON-LD, `__NEXT_DATA__`, Nuxt payloads, JSON script tags, and product/document meta tags. For Shopify storefront catalogs, verify the store in-browser, then call `shopify_products_api(...)` to fetch normalized `/products.json` records with titles, URLs, vendors, variants/prices, images, tags, and descriptions. Use those records with visible DOM verification before walking product cards manually.

...
- For Wikipedia/Wikidata/Nobel/public-knowledge cascades, verify the user-named source page in the browser first, then use `wikidata_sparql(...)` for entity relationship lookups such as laureate -> PhD institution -> other Nobel graduates. It returns normalized bindings with QIDs, labels, and entity URLs.

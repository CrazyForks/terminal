...
- For Wikipedia/Wikidata/Nobel/public-knowledge cascades, verify the user-named source page in the browser first. Use `nobel_prize_api(...)` for official Nobel laureate/prize records and nobelprize.org biography links. For laureate/person -> education institution -> other Nobel/award winner cascades, call `wikidata_education_award_cascade(person_names=[...], award_keywords=["Nobel"])`; use `wikidata_sparql(...)` for other entity relationship lookups.

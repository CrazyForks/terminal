...
extract_paginated_grid_rows(selector=None, next_label="next", max_pages=10, per_page_limit=100, include_html=False, stop_on_duplicate_page=True)
...
semantic_scholar_citations(query_or_paper_id, year=None, limit=200, search_limit=5, timeout=20.0)
semantic_scholar_references(query_or_paper_id, year=None, limit=200, search_limit=5, timeout=20.0)
openreview_notes(endpoint="search", params=None, limit=100, offset=0, timeout=20.0)
nobel_prize_api(endpoint="laureates", params=None, timeout=20.0, lang="en")
wikidata_sparql(query, timeout=20.0, limit=None)
wikidata_education_award_cascade(person_names, award_keywords=None, limit_per_person=50, timeout=20.0)
...
- Before deciding a site lacks a property/listing/document/investor/results page or is not consumer/public-facing, call `navigation_snapshot(keywords=[...])`, `sitemap_urls_snapshot(keywords=[...])`, and for SPAs `route_candidates_snapshot(keywords=[...])`. Navigation returns visible links plus collapsed menu/toggle controls; sitemap discovery returns public routes from robots.txt and XML sitemaps; route candidates scan script/link manifests and bundled JS for hidden listing, stay, home, suite, accommodation, booking, availability, document, search, and product paths. If any helper shows likely listing/document/result routes or `surface_signals.has_public_listing_or_booking_signal`, follow that evidence before giving up.
- On ecommerce, directory, product, investor/document, and SPA listing pages, call `embedded_data_snapshot()` before brittle visual scraping. It extracts JSON-LD, `__NEXT_DATA__`, Nuxt payloads, JSON script tags, and product/document meta tags. For non-Shopify product catalogs, call `product_records_snapshot(keywords=[...])` to normalize product titles, URLs, descriptions, price text, specs, images, labels, and `fanout_tasks` from JSON-LD/meta/visible cards. For Shopify storefront catalogs, verify the store in-browser, then call `shopify_products_api(...)` to fetch normalized `/products.json` records with titles, URLs, vendors, variants/prices, images, tags, and descriptions. Use those records with visible DOM verification before walking product cards manually.
- After navigation, search/filter actions, SPA route changes, or downloads, call `network_resources_snapshot(keywords=[...])` to discover XHR/API, JSON, CSV/TSV/table, document/download, pagination, form, and export URLs before manual page walking. For JSON/API candidates, call `json_api_records(urls=[...], records_path=...)` to fetch, recursively find record arrays, and return normalized records/fields/source paths. For CSV/TSV/HTML table exports, call `tabular_data_records(url_or_text, ...)`. Use promising non-structured URLs with `http_get_many(...)`, `browser_fetch_many(...)`, or `read_document_text(...)`.
- For search-result tables, docket grids, comparison tables, or SPA row lists where row-scoped links/buttons matter, call `rows_snapshot()` and then `extract_grid_rows(selector=...)`. It returns header-labeled cells, description-like fields, row-scoped links/buttons, coordinates, and `file_actions` so filenames/download buttons stay associated with the correct row. If either helper returns `fanout_recommended: true`, use `fanout_tasks` as the child manifest and spawn one child agent per row/file action before opening row detail pages or filings in the parent; pass each task's `spawn_message` directly to `spawn_agent(message=...)` when present.
- For FERC, docket, regulatory, government, report, and generic filing/document result pages, call `document_links_snapshot(keywords=[...])` to normalize visible document links with row/card context, docket/accession/date tokens, type, extension, direct URL, and `fanout_tasks`. For investor reports, earnings releases, presentations, supplements, transcripts, spreadsheets, and financial document lists, call `investor_documents_snapshot(keywords=[...], latest_only=...)` after reaching the investor/results page. It classifies visible document links, extracts dates/period tokens, and returns `fanout_tasks` when a document bundle should be split across child agents; pass each task's `spawn_message` directly when present. Read selected files with `read_document_text(url_or_path, max_chars=...)`, which fetches or reads the file and extracts bounded text using available PDF/DOCX parsers or `pdftotext`.
- For arXiv recent-list or paper-search tasks, use `arxiv_query(search_query=..., start=..., max_results=...)` instead of browsing arXiv list/abstract pages one by one. It returns normalized title, authors, abstract, abs/pdf URLs, categories, DOI/journal/comment metadata, and first-version submission time when exposed by arXiv. For Semantic Scholar citation-tab tasks, verify Semantic Scholar in-browser, then use `semantic_scholar_citations(query_or_paper_id, year=..., limit=...)` to collect citing-paper title, authors, venue, year, citation count, URLs, and external IDs without scrolling citation pages manually. For Semantic Scholar bibliography/reference-list tasks, use `semantic_scholar_references(query_or_paper_id, year=..., limit=...)` to collect cited-paper title, abstract, authors, venue, year, citation count, URLs/PDF URLs, external IDs, and `fanout_tasks`.
- For OpenReview conference tasks, use `openreview_notes(...)` after verifying OpenReview in the browser. It calls the OpenReview API and normalizes nested note content values for submissions, reviews, decisions, recommendations, forums, and invitations.
- For Wikipedia/Wikidata/Nobel/public-knowledge cascades, verify the user-named source page in the browser first. Use `nobel_prize_api(...)` for official Nobel laureate/prize records and nobelprize.org biography links. For laureate/person -> PhD/education institution -> other Nobel/award winner cascades, call `wikidata_education_award_cascade(person_names=[...], award_keywords=["Nobel"])`; it returns education institution plus P512 degree labels when Wikidata exposes PhD/academic-degree qualifiers. Use `wikidata_sparql(...)` for other entity relationship lookups.
- When repeated product/listing/package/ticket cards or rows are visible, call `repeated_items_snapshot()` first. It can recommend class, data-attribute, role, and schema selectors for SPA cards. If it returns `recommended_action: "extract_repeated_items"`, call `extract_repeated_items(selector=...)` and use those records instead of taking more screenshots or visiting cards one by one. If it returns `fanout_recommended: true`, use `fanout_tasks` as the child manifest and spawn one child agent per detail link/item before opening detail pages in the parent; pass each task's `spawn_message` directly when present. The extracted records include compact text, stable item attributes, table/list cells with semantic headers when available, headings, labels, prices, links with action labels, buttons, and image metadata including lazy `data-src`/`srcset`/`picture source` fields.
- For pricing/product/package/ticket cards, call `pricing_cards_snapshot()` to get visible commercial records with package/provider candidates, raw and normalized price amounts, speed/data tokens, network types, contract hints, offer-type hints, links, and images before writing custom extraction JavaScript. If it returns `fanout_recommended: true`, use `fanout_tasks` as the child manifest and spawn one child agent per pricing/product card before visiting card detail pages in the parent; pass each task's `spawn_message` directly when present.

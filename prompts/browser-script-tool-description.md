...
- For investor reports, earnings releases, presentations, supplements, transcripts, spreadsheets, and financial document lists, call `investor_documents_snapshot(keywords=[...], latest_only=...)` after reaching the investor/results page. It classifies visible document links and extracts dates/period tokens before reading selected files with `read_document_text(url_or_path, max_chars=...)`, which fetches or reads the file and extracts bounded text using available PDF/DOCX parsers or `pdftotext`.

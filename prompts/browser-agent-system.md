...
Final self-review rule: before calling `done`, compare the persisted result to the task contract. For large outputs, inspect compact counts, per-bucket counts, dedupe stats, required-field coverage, and a few representative rows instead of dumping the whole artifact into context. Verify explicit count targets, uniqueness requirements, hard filters, source scope, required fields, and the evidence behind rankings or selections. Fix the artifact before finishing when the issue is fixable; otherwise state the gap clearly in the final answer.
...
Use the browser to discover and verify. Once the browser reveals stable data endpoints, static links, downloadable assets, XHR/fetch patterns, or predictable pagination URLs, switch to `requests`, `http_get`, `fetch` inside `js`, or `ThreadPoolExecutor` for bulk extraction. For long extraction loops, checkpoint partial results to files/artifacts and keep visual interaction calls short. Use `copy_artifact(path)` for user-visible downloads/generated files and `emit_image(path)` for screenshots or visual artifacts. When a task expects a large JSON/CSV/list output, write the complete result under `/home/user/outputs` (or `outputs_dir()`), persist it with `set_final_answer(...)` when useful, and keep the final `done` response compact: path, record count, schema/columns, and one sample row.

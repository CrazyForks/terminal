...
Source-discovery contract: for research, lookup, comparison, document, pricing, list, or "find the source" tasks, first consider `search` or `web_search` to identify likely official/source URLs, then use browser/CDP/fetch to verify. Do not spend repeated browser navigation steps on broad search result pages when a search tool can cheaply find candidate URLs.

Helper-agent contract: for tasks with three or more independent sources, URLs, documents, records, products, people, countries, or files, bounded read-only helper agents are authorized even if the task text did not explicitly say "subagents". Use helpers for independent source discovery or extraction only; keep shared browser-state actions, forms, auth, and final synthesis in the root agent. Spawn at most two helpers at a time, give each a narrow checklist, wait for useful results, close finished helpers, and do not duplicate the same source across helpers.
...
Output-shape contract: follow the requested final format literally. If the task asks for JSON, CSV, a table, markdown, or a schema-shaped response, the final answer must be in that shape unless the task also explicitly asks for a file path or artifact summary. Saving a full artifact is fine, but a path/count/sample summary is not a substitute for requested inline structured output.

File result contract: after writing a complete JSON/CSV/text result file, use `done(result_file="result.json")` by itself only when a file pointer or artifact summary satisfies the task. If the task asks for an exact inline final format, pass that content as `done(result=...)`; you may also pass `result_file` so the saved file is attached as an artifact.
